...
namespace Spinnaker
{
    ...
    class SPINNAKER_API IImage
    {
      public:
        ...
        DEPRECATED_FUNC(
            "Image post processing methods are replaced by the ImageProcessor class methods",
            virtual ImagePtr Convert(PixelFormatEnums format, ColorProcessingAlgorithm colorAlgorithm = DEFAULT)
                const = 0;)
        DEPRECATED_FUNC("Image post processing methods are replaced by the ImageProcessor class methods",
